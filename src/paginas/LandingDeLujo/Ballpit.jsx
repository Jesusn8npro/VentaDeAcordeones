/* eslint-disable */
// Ballpit — componente de React Bits (https://reactbits.dev), inspirado en el trabajo de
// Kevin Levron (https://x.com/soju22/status/1858925191671271801).
//
// Se guarda como .jsx a propósito: es código de terceros minificado a mano (campos privados #x,
// nombres de una letra) y no tiene sentido re-tipar ni reformatear algo que se actualiza pegando la
// versión nueva de la web. El tsconfig del proyecto tiene allowJs, así que entra tal cual.
//
// ⚠️ ÚNICA MODIFICACIÓN sobre el original, marcada más abajo: se quitaron los `e.preventDefault()`
// de los handlers de touch. En el original se registran en document.body y cancelan TODOS los
// touchmove de la página — en esta landing, que es scrollytelling puro, eso dejaba el móvil sin
// poder hacer scroll en cuanto el componente se montaba. Sin ellos la bola sigue el dedo igual; lo
// único que se pierde es el bloqueo del scroll mientras se arrastra dentro del lienzo, que aquí es
...
function TouchStart(e) {
  if (e.touches.length > 0) {
    // (modificado) sin preventDefault: cancelaba el scroll de toda la página en móvil.
...
function TouchMove(e) {
  if (e.touches.length > 0) {
    // (modificado) sin preventDefault: ver la nota de la cabecera.
    A.x = e.touches[0].clientX;
    A.y = e.touches[0].clientY;
  ...
  constructor(e) {
    ...
    this.onBeforeCompile = e => {
      ...
      e.fragmentShader = e.fragmentShader.replace(
        'void main() {',
        // (modificado) `vColor` → `vec3(vColor)`. Desde three r152 esa variable se declara vec4
        // cuando hay color por instancia — que es justo lo que usa el ballpit —, así que la
        // multiplicación original no compilaba: "dimension mismatch: cannot convert from
        // 4-component vector of float". El constructor vec3() vale para las dos declaraciones.
        '\n        void RE_Direct_Scattering(const in IncidentLight directLight, const in vec2 uv, const in vec3 geometryPosition, const in vec3 geometryNormal, const in vec3 geometryViewDir, const in vec3 geometryClearcoatNormal, inout ReflectedLight reflectedLight) {\n          vec3 scatteringHalf = normalize(directLight.direction + (geometryNormal * thicknessDistortion));\n          float scatteringDot = pow(saturate(dot(geometryViewDir, -scatteringHalf)), thicknessPower) * thicknessScale;\n          #ifdef USE_COLOR\n            vec3 scatteringIllu = (scatteringDot + thicknessAmbient) * vec3(vColor);\n          #else\n            vec3 scatteringIllu = (scatteringDot + thicknessAmbient) * diffuse;\n          #endif\n          reflectedLight.directDiffuse += scatteringIllu * thicknessAttenuation * directLight.color;\n        }\n\n        void main() {\n      '
      );
...
function createBallpit(e, t = {}) {
  ...
  // (modificado) sin touchAction: 'none'. Con él, el navegador no deja hacer scroll con el dedo
  // sobre el lienzo, y aquí el lienzo ocupa toda la sección de cierre.
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // (modificado) El lienzo se crea aquí y se DESTRUYE en la limpieza, en vez de venir de un ref
    // fijo del JSX. `dispose()` llama a `renderer.forceContextLoss()`, que inutiliza el contexto
    // WebGL de ese <canvas> para siempre; como React 19 en StrictMode monta, limpia y vuelve a
    // montar, el segundo WebGLRenderer recibía un contexto null sobre el mismo elemento y reventaba
    // con "Cannot read properties of null (reading 'precision')". Un lienzo nuevo por montaje lo
    // resuelve de raíz, y de paso cubre cualquier remontaje futuro (navegación, hot reload).
    ...
    // (modificado) Un resize forzado nada más crearlo. `createBallpit` asigna `onAfterResize` —el
    // callback que ajusta los límites `maxX`/`maxY` de la física al tamaño real del lienzo— DESPUÉS
    // de su última llamada a resize(), así que las bolas se quedaban encerradas en la caja por
    // defecto de 10×10 unidades: un bloque estrecho en mitad del panel, con los lados vacíos y
    // desbordando por arriba y por abajo. Hasta que el usuario redimensionaba la ventana no se
    // corregía solo.
  ...
  // El contenedor es además el `parent` del que createBallpit lee el tamaño (size: 'parent').
