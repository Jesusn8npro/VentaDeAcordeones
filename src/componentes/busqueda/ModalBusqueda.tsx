...
export default function ModalBusqueda({ abierto, onCerrar }) {
  ...
  const buscarProductos = async () => {
    setCargando(true)
    const sel = `id,nombre,precio,slug,descripcion,marca,modelo,palabras_clave,categorias(id,nombre,slug),producto_imagenes(imagen_principal,imagen_secundaria_1)`
    try {
      const q = terminoBusqueda.trim()
      const like = `%${q}%`
      const base = () => clienteSupabase.from('productos').select(sel).eq('activo', true).gt('stock', 0).limit(6)
      const [{ data: r1 }, { data: r2 }, { data: r3 }] = await Promise.all([
        base().or(`nombre.ilike.${like},descripcion.ilike.${like},marca.ilike.${like},modelo.ilike.${like},slug.ilike.${like}`),
        base().contains('palabras_clave', [q]),
        base().ilike('palabras_clave', like)
      ])
      const todos = [...(r1 || []), ...(r2 || []), ...(r3 || [])]
      setProductos(todos.filter((p, i, self) => i === self.findIndex(x => x.id === p.id)).slice(0, 6))
    } catch {
    } finally {
      setCargando(false)
    }
