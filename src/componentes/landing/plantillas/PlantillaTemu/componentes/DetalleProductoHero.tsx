...
const DetalleProductoHero = ({
  ...
  // El descuento se calcula con los precios de la base; nunca se escribe a mano.
  const hayRebaja = Number(producto?.precio_original) > Number(producto?.precio) && Number(producto?.precio) > 0
  const rebajaPct = hayRebaja
    ? Math.round(((producto.precio_original - producto.precio) / producto.precio_original) * 100)
    : 0

  return (
    <div className="hero-temu-columna-derecha">
      {/* La banda de oferta se pinta sólo si hay descuento de verdad. Antes salía "MEGA OFERTA"
          en todas las fichas, más un "$4.000 de crédito por retraso" que nadie ofrece aquí. */}
      {Number(producto?.descuento) > 0 && (
        <div className="hero-temu-banner-mega-oferta">
          <div className="hero-temu-etiqueta-mega">EN<br/>OFERTA</div>
          <div className="hero-temu-beneficios-container">
            <span className="hero-temu-beneficio-texto"><span className="hero-temu-check-verde">✓</span> Envío gratis desde $50.000</span>
            <span className="hero-temu-beneficio-texto"><span className="hero-temu-check-verde">✓</span> Garantía de 12 meses</span>
          </div>
        </div>
      )}
      ...
      {/* Sin reseñas reales no hay estrellas: el "(1.559 opiniones)" de antes era un literal. */}
      ...
      <div className="hero-temu-opiniones-verificado mobile-order-1" aria-label="Opiniones y verificación">
        <span className="hero-temu-opiniones-count">
          ({Number(producto?.total_resenas || 0).toLocaleString('es-CO')} opiniones)
        </span>
      ...
      <div className="hero-temu-seccion-precios-vendedora mobile-order-3">
        ...
        <div className={`hero-temu-precio-principal-container ${producto?.activo === false ? 'vendido' : ''}`}>
          <span className="hero-temu-precio-actual-grande">{formatearPrecioCOP(producto?.precio || 0)}</span>
          {hayRebaja && (
            <>
              <span className="hero-temu-precio-original-tachado">{formatearPrecioCOP(producto.precio_original)}</span>
              <span className="hero-temu-ahorro-label">Ahorras: {formatearPrecioCOP(producto.precio_original - producto.precio)}</span>
              <span className="hero-temu-badge-descuento-grande">-{rebajaPct}% de descuento</span>
            </>
          )}
        </div>
      </div>
      ...
      {/* La "oferta relámpago" se pinta sólo si el producto tiene un descuento de verdad. Antes
          salía en todas las fichas con un reloj en cuenta atrás para una oferta que no existía
          y que, al llegar a cero, volvía a empezar. */}
      {producto?.estado !== 'vendido' && Number(producto?.descuento) > 0 && (
