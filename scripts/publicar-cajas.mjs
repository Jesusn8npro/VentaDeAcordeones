...
const CIERRE =
  ' Es la misma línea que usan los cajeros de los grandes del vallenato: los de Silvestre Dangond, Peter Manjarrés y Elder Dayán. Casco torneado y lacado a mano, aros y tensores cromados, parche acrílico afinable con llave incluida. Fabricada a mano en Colombia por maestros percusionistas. La tienes en Bogotá para entrega inmediata y la enviamos asegurada a cualquier ciudad del país o al exterior. Pídela con estuche acolchado y guacharaca en combo.'

const CAJA_BASE = {
  precio: 750000, stock: 3, marca: 'Artesanal Colombiana', modelo: 'Profesional', categoriaSlug: 'cajas-vallenatas',
  claves: ['caja vallenata', 'caja vallenata profesional', 'comprar caja vallenata', 'caja vallenata bogotá', 'caja vallenata precio', 'percusión vallenata'],
}

const CAJAS = [
  { slug: 'caja-vallenata-profesional-sandro-verde-esmeralda', nombre: 'Caja Vallenata Profesional Verde Esmeralda', foto: '03d9af95-f184-442e-b2f6-b8837d094a48.jpg', color: 'Verde esmeralda',
  ...
  { slug: 'caja-vallenata-profesional-sandro-madera-veteada', nombre: 'Caja Vallenata Profesional Madera Veteada', foto: '21f6aed4-7d0d-4984-aab4-9a7ec24617f2.jpg', color: 'Madera veteada natural',
  ...
  { slug: 'caja-vallenata-profesional-sandro-degradado-rojo-negro', nombre: 'Caja Vallenata Profesional Degradado Rojo y Negro', foto: '319b8cce-b9e6-4d8a-878b-afe1d290a7c5.jpg', color: 'Degradado rojo y negro',
  ...
  { slug: 'caja-vallenata-profesional-sandro-tricolor-colombia', nombre: 'Caja Vallenata Profesional Tricolor Colombia', foto: 'a26ad0fc-40ec-4ab6-830d-8711837361f3.jpg', color: 'Amarillo, azul y rojo',
  ...
  { slug: 'caja-vallenata-profesional-sandro-dorada', nombre: 'Caja Vallenata Profesional Dorada', foto: '9e286d4a-a618-4ee6-95c6-77cfc9edc0a3.jpg', color: 'Dorado metalizado',
  ...
  { slug: 'caja-vallenata-profesional-sandro-degradado-verde-azul', nombre: 'Caja Vallenata Profesional Degradado Verde y Azul', foto: 'ca9fc2b5-d2ec-4189-a60e-73d693e32022.jpg', color: 'Degradado verde y azul',
  ...
  { slug: 'caja-vallenata-profesional-sandro-azul-rey', nombre: 'Caja Vallenata Profesional Azul Rey', foto: '7d73dbf1-2054-47a6-abb3-47a69073c4fe.jpg', color: 'Azul rey',
  ...
  { slug: 'caja-vallenata-profesional-sandro-personalizada-con-nombre', nombre: 'Caja Vallenata Profesional Personalizada con tu Nombre', foto: 'e4b8f8ee-189d-47a3-8417-fffb6eef93b8.jpg', foto2: '71c7c0bd-184c-493e-af3f-309242cb0e1d.jpg', color: 'Madera natural (color a elección)',
    gemini: 'la caja vallenata (tambor cónico) de madera clara con aros cromados y unas iniciales pintadas en el parche', intro: 'Tu nombre, tus iniciales o el de tu agrupación pintados a mano en el parche, y el color de casco que elijas.',
...
for (const c of CAJAS) {
  ...
  try {
    ...
    const contenido = `${c.intro} ${c.nombre.replace('Caja Vallenata Profesional', 'Caja vallenata profesional')}: casco de madera torneado y lacado a mano en ${c.color.toLowerCase()}.` + CIERRE
    const p = { ...CAJA_BASE, ...c }
